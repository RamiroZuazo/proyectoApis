...
export const getGastosPendientes = async (usuarioResponsableId, usuarioDeudorId, proyectoId) => {
    ...
    console.log("Datos enviados para obtener los gastos pendientes:", { usuarioResponsableId, usuarioDeudorId, proyectoId }); // Debug

    // Formamos la URL con los parámetros correspondientes
    const url = `http://localhost:8080/api/gastos-pendientes/${usuarioResponsableId}/${usuarioDeudorId}/${proyectoId}`;
    ...
    if (!response.ok) {
        const errorData = await response.json();
        console.error("Error al obtener los gastos pendientes:", errorData); // Debug
